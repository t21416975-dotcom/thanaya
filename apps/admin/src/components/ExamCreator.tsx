    ...
    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
        ...
        <div className="flex items-center gap-1 bg-slate-200 p-1 rounded-xl text-xs font-semibold w-full sm:w-auto">
          {!initialExam && (
            <button
              ...
              className={`flex-1 sm:flex-initial flex items-center justify-center gap-1.5 py-2 px-3 rounded-lg transition cursor-pointer text-center ${
                activeTab === 'ai-upload' ? 'bg-emerald-600 text-white shadow-sm' : 'text-slate-600 hover:text-slate-900'
              }`}
            >
              <Sparkles className="w-3.5 h-3.5" />
              <span>استخراج بـ AI</span>
            </button>
          )}
          <button
            type="button"
          ...
          >
            ...
            <span>محرر الأسئلة ({questions.length})</span>
          </button>
        </div>
      </div>
      ...
      {activeTab === 'editor' && (
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
          ...
          {/* Top Exam Meta Bar */}
          <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200">
          ...
          <div className="space-y-6">
            {questions.map((q, qIndex) => (
              <div
                key={qIndex}
                className="bg-white border border-slate-200 hover:border-slate-300 rounded-2xl p-4 sm:p-6 shadow-sm space-y-4 transition"
              >
                {/* Question Header */}
                      ...
                      return (
                        ...
                        >
                          ...
                          <input
                            ...
                            className="min-w-0 flex-1 bg-white px-3 py-2 border border-slate-300 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500"
                          />
                          {isCorrect && (
          ...
          <div className="flex flex-col-reverse sm:flex-row items-center justify-between gap-3 pt-6 border-t border-slate-200">
            ...
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-8 py-3 min-h-[44px] rounded-xl text-sm font-bold transition shadow-sm disabled:opacity-50 cursor-pointer"
            >
