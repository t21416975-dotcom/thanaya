...
import { Bot, Sparkles, Save, RotateCcw, CheckCircle2, AlertCircle, Cpu, Key, HelpCircle } from 'lucide-react';
...
export function AISettingsManager() {
  ...
  useEffect(() => {
    if (settings && settings.length > 0) {
      ...
      const key = settings.find((s) => s.key === 'gemini_api_key');
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* API Key (Optional Override) Card */}
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="font-bold text-slate-900 flex items-center gap-2 text-sm">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1.5 leading-relaxed">
              إذا تم تعيين مفتاح <code className="font-mono bg-slate-100 px-1 py-0.5 rounded">GEMINI_API_KEY</code> في متغيرات بيئة Vercel، يمكنك ترك هذه الخانة فارغة.
            </p>
          </div>
        </div>
